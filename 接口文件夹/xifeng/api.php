...
class JosnData
{
	...
	public function get_downbook()
	{
		...
		file_put_contents($filename,$this->trans(trim($pid['subject'])."\r\n".$tmp."\r\n"));
...
public function get_myguanzhu()
{
	...
	{
		$user = $this->utf2gbk($this->params['username']);
		$user = addslashes($user);
		$pass = addslashes(trim($this->utf2gbk($this->params['pass'])));
	...
	{
		global $_G;
		$username = $this->utf2gbk($this->params['username']);
		$username = daddslashes($username);
		$password = daddslashes($this->utf2gbk($this->params['pass']));
	...
	public function get_fastreg($dqlogintype='',$openid='',$nickname='')
	{
		...
		$nickname = $this->utf2gbk($nickname);
		require_once './../uc_client/client.php';
		require_once './../source/class/class_member.php';
		require_once './../source/function/function_member.php';
		$uid = uc_user_register($username, $password, $email, '', '', $_G['clientip']);
	...
	public function get_postshuping()
	{
		...
		$text = $this->utf2gbk($this->params['text']);
	...
	function utf2gbk($str){
		// if($this->_isutf8($str) && (strtolower(CHARSET) !='utf-8')){
		// 	return diconv($str,'utf-8','gbk');
		// }else if(strtolower(CHARSET) == 'utf-8'){
		// 	return diconv($str,'gbk','utf-8');
		// }
		return $str;
	}
