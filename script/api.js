...
function ajaxuser() {
    ...
    userinfo.auth = userinfo.auth?userinfo.auth:0;
    var cookieauth = 0;
    document.cookie.split(',').forEach(function(s){
        var tmp = s.split('=')
        if((tmp[0]==appinfo.cookiepre+'auth') && !cookieauth){
           cookieauth = tmp[1]
        }
    })
    if (userinfo.bbsuid || cookieauth)
        api.ajax({
            url: appinfo.url + 'api.php?appkey=' + appinfo.appkey + '&type=get&module=user&param=bbsuid----' + userinfo.bbsuid+',auth----'+userinfo.auth+',autokey----'+cookieauth,
            method: 'get'
        }, function(ret, err) {
            if (ret) {
                if(ret.error && ret.error=='error'){
                    return;
                }
...
function autocheckyejian() {
    if ($api.byId('yejiancss')) {
        if ($api.getStorage('yejianmoshi') == '1') {
            $api.attr($api.byId('yejiancss'), 'href', '../css/aui-skin-night.css');
        } else {
            $api.removeAttr($api.byId('yejiancss'), 'href');
        }
    } else if ($api.byId('yejiancssindex')) {
        if ($api.getStorage('yejianmoshi') == '1') {
          api.execScript({
              name: 'root',
              script: "$api.attr($api.byId('yejiancssindex'), 'href', './css/aui-skin-night.css');"
          });
        }else{
          api.execScript({
              name: 'root',
              script: "$api.removeAttr($api.byId('yejiancssindex'), 'href');"
          });
        }
    }
}
